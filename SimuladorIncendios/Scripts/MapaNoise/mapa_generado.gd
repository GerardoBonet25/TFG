...
func _generar_matriz_3d():
	...
	#Escaneamos la superficie (X, Z) para construir las columnas (Y)
	...
	for z in range(resolucion_matriz):
		for y in range(resolucion_y):
			for x in range(resolucion_matriz):
				...
				# Calculamos el índice de la GPU 
				var indice_gpu = (z * resolucion_matriz * resolucion_y) + (y * resolucion_matriz) + x
				
				# Traducimos los estados a bytes (0-255) para que el Shader entienda qué quema
	...
	if event is InputEventMouseButton and event.button_index == MOUSE_BUTTON_LEFT and event.pressed:
		...
		if impacto:
			...
			if cubo_frontal != null and controlador != null:
				# Convertimos el punto global del impacto al espacio local del cubo (de -0.5 a 0.5)
				var pos_local_cubo = cubo_frontal.to_local(punto_global)
				
				# Normalizamos al rango de 0.0 a 1.0
