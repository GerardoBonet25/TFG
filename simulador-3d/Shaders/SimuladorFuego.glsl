...
// Aqui recibimos los 16 bytes exactos que nos manda el script Controlador
...
void main() {
	...
	//Si tiene una coortdenada que se sale del mapa aborta la ejecucion de ese voxel
	...
	// Leemos si este voxel especifico es matorral/arbol (1.0) o aire/roca (0.0)
	float es_combustible = imageLoad(mapa_combustibles, coord).r;
	
	
	if (es_combustible < 0.5) {
			...
			for (int x = -1; x <= 1; x++) {
				...
				if (coord_vecino.x >= 0 && coord_vecino.x < tamano_mapa.x &&
					...
					if (estado_vecino > 0.5) {
						...
						// CALCULO DE VIENTO 3D
						// Usamos X y Z para la direccion del viento (plano horizontal)
						vec2 dir_bruto = vec2(float(-x), float(-z));
						float cos_theta = 0.0;

						// Evitamos el error de normalizar (0,0) si el vecino está justo arriba o abajo
						if (length(dir_bruto) > 0.01) {
							vec2 dir_vecino = normalize(dir_bruto); 
							cos_theta = dot(dir_vecino, vector_viento);
						} else {
							// Si el fuego se propaga verticalmente, la termodinámica dicta que sube rápido.
							// Le asignamos el valor máximo como si el viento soplara totalmente a favor.
							cos_theta = 1.0; 
						}
	...
	if (riesgo_ignicion > 0.0) {
		// Sumamos el valor logico .x a la semilla espacial.
